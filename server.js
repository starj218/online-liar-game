...
function broadcastRoom(r){
  ...
  io.to(r.id).emit("roomUpdate",{
    ...
    canStart:r.type==="code"&&r.state==="waiting"&&list.length>=4&&list.length<=8
  });
}
...
function abortIfTooSmall(r){
  if(!r||r.state==="waiting"||r.players.size>=4)return false;
  clearTimers(r);
  io.to(r.id).emit("roomAborted",{message:"플레이어가 3명 이하로 줄어 게임을 계속할 수 없습니다."});
...
function tryMatch(size){
  ...
  setTimeout(()=>{
    ...
    if(rr&&rr.state==="matched"&&rr.players.size>=4)startModeVote(rr);
  },1800);
...
function startModeVote(r){
  if(!r||r.players.size<4){abortIfTooSmall(r);return;}
  clearTimers(r);
  r.state="modeVote";
  r.modeVotes=new Map();
  r.selectedMode=null;
...
function readyTimeout(id){
  ...
  r.restartTimer=setTimeout(()=>{
    const rr=rooms.get(id);
    if(rr&&rr.players.size>=4)setupRoleStage(rr);
  },1800);
}
...
io.on("connection",socket=>{
  socket.on("joinWithNickname",(nickname,cb)=>{
    ...
    players.set(socket.id,{id:socket.id,nickname,status:"lobby",roomId:null,queueSize:null});
  ...
  socket.on("joinCodeRoom",(raw,cb)=>{
    ...
    if(r.players.size>=8)return cb({success:false,message:"이 방은 이미 8명으로 가득 찼습니다."});
  ...
  socket.on("joinMatchmaking",(size,cb)=>{
    ...
    size=Number(size);
    if(!p)return cb({success:false,message:"먼저 닉네임으로 입장해주세요."});
    if(![4,5,6,7,8].includes(size))return cb({success:false,message:"잘못된 매칭 인원입니다."});
  ...
  socket.on("leaveRoom",cb=>{
    if(players.has(socket.id))removeFromCurrent(socket.id);
    cb?.({success:true});
  });
  ...
  socket.on("requestGameStart",cb=>{
    ...
    if(r.players.size<4)return cb({success:false,message:"최소 4명이 필요합니다."});
